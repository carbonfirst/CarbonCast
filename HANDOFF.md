...
- **Data and trained models are NOT in git — they're local only.** Folders like
  `data/`, `data_old (v2.1)`, `data_old(v3.1)`, `CI_forecast_data/`, `real_time/`,
  and the `saved_*_models/` dirs are excluded via `.gitignore` because they're
  huge and re-fetchable. A fresh clone won't have them. See
  [`docs/05-data-and-large-files.md`](docs/05-data-and-large-files.md) for what's
  excluded and how to get it back.
