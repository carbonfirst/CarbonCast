...
To run CarbonCast end-to-end, run the following command:

```python
python3 run.py <REGION> <date>
```

The `<date>` must be in the format *YYYY-MM-DD*. For example, to run CarbonCast for the region *CISO* on *2021-07-01*, run the following command:

```python
python3 run.py CISO 2023-05-22
```

### 5.2 Componentized Run

CarbonCast can also be run in a componentized manner. The following sections describe the steps to run each component.

#### 5.2.1 Getting Weather data:
...
### 5.2.2 Getting source production forecasts:
...
### 5.2.3 Calculating carbon intensity (real-time/historical/from source production forecasts):
...
### 5.2.4 Getting carbon intensity forecasts using CarbonCast:
