...
def startScript(balAuthList, startDate, numDays, dayJump, outputFileDir, isRealTime=False):
    ...
    for balAuth in balAuthList:
        ...
        filedir = os.path.normpath(os.path.join(parentdir, outputFileDir+balAuth+"/solar_wind_forecasts/"))
        ...
        csv_path = filedir+"/"+balAuth+"_solar_wind_forecasts_raw.csv"
        ...
        partial_clean_csv_path = filedir+"/"+balAuth+"_solar_wind_forecasts_partial_clean.csv"
        ...
        clean_csv_path = filedir+"/"+balAuth+"_solar_wind_forecasts.csv"
...
if __name__ == "__main__":
    if (len(sys.argv)!=4):
        print("Usage: python3 entsoeSolarWindForecastParser.py <region> <yyyy-mm-dd> <# days to fetch data for>")
        print("region: Specify a particular region, or \"all\" to fetch data for all EU regions.")
        print("Check config/code.readme for currently supported EU regions")
        exit(0)

    regions = sys.argv[1]
    startDate = sys.argv[2] # "2022-01-01" #"2019-01-01"
    numDays = int(sys.argv[3]) #368 #1096

    if (regions == "all"):
        regions = ENTSOE_BAL_AUTH_LIST
    else:
        regions = regions.split(" ")

    startScript(regions, startDate, numDays, dayJump=1, 
                outputFileDir="./data/EU_DATA/", isRealTime=False)
