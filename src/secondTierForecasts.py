...
def runSecondTier(configFileName, cefType, loadFromSavedModel):
    ...
    for region in regionList:
        ...
        if (
                "NUM_SPECIALIZED_FORECAST_FEATURES" in regionConfig):  # weather + subset of source production forecasts
            ...
            for source in specializedForecasts:
                modifiedForecastDataset["avg_" + source.lower() + "_production_forecast"] = \
                forecastDataset["avg_" + source.lower() + "_production_forecast"]
        ...
        trainData, valData, testData, ftMin, ftMax = common.scaleDataset(trainData, valData,
                                                                         testData)
        print(trainData.shape, valData.shape, testData.shape)
        wTrainData, wValData, wTestData, wFtMin, wFtMax = common.scaleDataset(wTrainData, wValData,
                                                                              wTestData)
        ...
        for exptNum in range(NUMBER_OF_EXPERIMENTS):
            ...
            regionDailyMape = {}
            bestModel, numFeaturesInTraining = trainingandValidationPhase(region, trainData,
                                                                          wTrainData,
                                                                          valData, wValData,
                                                                          secondTierConfig, exptNum,
                                                                          loadFromSavedModel)
            ...
            predictedData = getDayAheadForecasts(bestModel, history, testData,
                                                 TRAINING_WINDOW_HOURS, numFeaturesInTraining,
                                                 DEPENDENT_VARIABLE_COL,
                                                 wFtMin[5:], wFtMax[5:],
                                                 ftMin[DEPENDENT_VARIABLE_COL],
                                                 ftMax[DEPENDENT_VARIABLE_COL],
                                                 wTestData, weatherData,
                                                 forecastDataset.columns.values[
                                                 :numForecastFeatures])
            print("***** Forecast done *****")
            ...
            modTestData = manipulateTestDataShape(testData, MODEL_SLIDING_WINDOW_LEN,
                                                  PREDICTION_WINDOW_HOURS, False)
            modTestData = np.reshape(modTestData, (
            modTestData.shape[0] * modTestData.shape[1], modTestData.shape[2]))
            ...
            for i in range(0, PREDICTION_WINDOW_HOURS // 24):
                ...
                mapeByDay.append([i + 1, np.mean(regionDailyMape[region][:, i]),
                                  np.percentile(regionDailyMape[region][:, i], 50),
            ...
            with open("../data/" + region + "/" + region + "_MAPE_iter" + str(exptNum) + ".txt",
                      "w") as f:
            ...
            if (writeCIForecastsToFile == "True"):
                common.writeOutFile(outFileNamePrefix + "_" + str(exptNum) + ".csv", data,
                                    "carbon_intensity", "w")
...
def runSecondTierInRealTime(configFileName, regionList, cefType, startDate, electricityDataDate,
    ...
    for region in regionList:
        ...
        if (cefType == "-l"):
            ...
            print("Direct CEF")
        regionConfig = secondTierConfig[region]
        inFileName = realTimeFileDir + region + "/" + region + "_" + str(
            electricityDataDate) + "_direct_emissions.csv"
        outFileName = realTimeFileDir + region + "/" + region + "_direct_CI_forecasts_" + str(
            startDate) + ".csv"
        if (cefType == "-l"):
            inFileName = realTimeFileDir + region + "/" + region + "_" + str(
                electricityDataDate) + "_lifecycle_emissions.csv"
            outFileName = realTimeFileDir + region + "/" + region + "_lifecycle_CI_forecasts_" + str(
                startDate) + ".csv"
        forecastInFileName = realTimeForeCastFileName[region]
        numHistoricalAndDateTimeFeatures = secondTierConfig["NUM_FEATURES"]
        numForecastFeatures = regionConfig["NUM_FORECAST_FEATURES"]
        startCol = secondTierConfig[
                       "START_COL"] + 2  # because we have added creation time & version for real-time

        print("Initializing...")
        print(inFileName, forecastInFileName)
        dataset, testDates, forecastDataset = initializeInRealTime(inFileName, forecastInFileName,
                                                                   startCol)
        print("***** Initialization done *****")
        ...
        testData = fillMissingData(testData)
        wTestData = np.array(
            forecastDataset.values[:, 0:numHistoricalAndDateTimeFeatures + numForecastFeatures - 1])
        wTestData = fillMissingData(wTestData)
...
def initialize(inFileName, forecastInFileName, startCol):
    ...
    forecastDataset = pd.read_csv(forecastInFileName, header = 0, infer_datetime_format = True,
                                  ...
                                  index_col = ['datetime'])  # new data files in data
...
def initializeInRealTime(inFileName, forecastInFileName, startCol):
    ...
    forecastDataset = forecastDataset.iloc[:,
                      2:]  # because we have added creation time & version for real-time
...
def manipulateTrainingDataShape(data, trainWindowHours, labelWindowHours, weatherData = None):
    ...
    for i in range(len(data) - (trainWindowHours + labelWindowHours) + 1):
        # skip if weather data slice is incomplete
        if weatherIdx + trainWindowHours > len(weatherData):
            print(f"[WARNING] Skipping at i={i} due to short weather data slice.")
            break  # or use `continue` to skip instead of exiting the loop
...
def trainModel(trainX, trainY, valX, valY, hyperParams, iteration, region, loadFromSavedModel):
    ...
    print("Timesteps: ", n_timesteps, "No. of features: ", n_features, "No. of outputs: ",
          n_outputs)

    if (loadFromSavedModel is True):
        print("-s parameter specified. Loading model from ",
              SAVED_MODEL_LOCATION + region + "/" + region + ".h5")
    ...
    mc = ModelCheckpoint(SAVED_MODEL_LOCATION + region + "/" + region + ".h5", monitor = 'val_loss',
                         mode = 'min', verbose = 1, save_best_only = True)
    rlr = ReduceLROnPlateau(monitor = "val_loss", mode = "min", factor = 0.1, patience = 6,
                            verbose = 1, min_lr = minLearningRate)
...
def showModelSummary(history, model, architecture = None):
    ...
    print(
        history.history.keys())  # ['loss', 'mean_absolute_error', 'val_loss', 'val_mean_absolute_error']
    return
...
def getDayAheadForecasts(model, history, testData,
    ...
    for i in range(0, ((len(testData) // 24) - (BUFFER_HOURS // 24))):
        ...
        history.extend(
            testData[currentDayHours:currentDayHours + MODEL_SLIDING_WINDOW_LEN, :].tolist())
...
def findImportantFeatures(model, valData, featureList, testDates):
    ...
    topNFeatures = {}
    featureImp = {}
    valDataReshaped = np.reshape(valData, (
    valData.shape[0] // MODEL_SLIDING_WINDOW_LEN, MODEL_SLIDING_WINDOW_LEN, valData.shape[1]))
...
def fillMissingData(
        data):  # If some data is missing (NaN), use the same value as that of the previous row
...
def trainingandValidationPhase(region, trainData, wTrainData, valData, wValData, secondTierConfig,
    ...
    bestTrainedModel, numFeatures = trainModel(X, y, valX, valY, hyperParams, exptNum, region,
                                               loadFromSavedModel)
    print("***** Training done *****")
...
def getUnscaledForecastsAndForecastAccuracy(testData, testDates, predictedData, ftMin, ftMax):
    ...
    formattedTestDates = manipulateTestDataShape(testDates,
                                                 MODEL_SLIDING_WINDOW_LEN, PREDICTION_WINDOW_HOURS,
                                                 True)
    ...
    unscaledTestData = common.inverseDataScaling(actual, ftMax[DEPENDENT_VARIABLE_COL],
                                                 ftMin[DEPENDENT_VARIABLE_COL])
    print(actualData.shape, predictedData.shape, unscaledTestData.shape,
          unscaledPredictedData.shape)
...
if __name__ == "__main__":
    ...
    if (len(sys.argv) < 3):
        print(
            "Usage: python3 secondTierForecasts.py <configFileName> <-l (lifecycle)/ -d (direct)> <-s>")
