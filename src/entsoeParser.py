...
if __name__ == "__main__":
    if (len(sys.argv)!=4):
        print("Usage: python3 entsoeParser.py <region> <yyyy-mm-dd> <# days to fetch data for>")
    ...
    for balAuth in regions:
        ...
        filedir = os.path.normpath(os.path.join(parentdir, f"./data/EU_DATA/"))
        ...
        cleanedDataset.to_csv(filedir+f"/{balAuth}_tmp_missing_data_cleaned.csv") # see if string suffices

        # adjust source columns
        dataset = pd.read_csv(filedir+f"/{balAuth}_tmp_missing_data_cleaned.csv", header=0, index_col=["UTC time"])
        modifiedDataset = adjustColumns(dataset, balAuth)
        modifiedDataset.to_csv(filedir+f"/{balAuth}_clean.csv")
