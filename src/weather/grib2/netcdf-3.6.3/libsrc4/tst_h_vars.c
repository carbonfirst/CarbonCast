/* This is part of the netCDF package.
   ...
   $Id: tst_h_vars.c,v 1.15 2007/12/31 17:03:25 ed Exp $
*/
#include <nc_tests.h>
...
#define DEFLATE_NAME "deflate"
   {
	 ...
	 ERR;
      ...
      if ((datasetid = H5Dopen(grpid, VAR_BOOL_NAME)) < 0) ERR;
   ...
   SUMMARIZE_ERR;
   printf("*** Checking HDF5 deflatfe, fletcher32, shuffle filter setting and getting...");
   {
      H5Z_filter_t filter;
      int num_filters;
      hid_t propid;
...
#define BE_VAR_NAME "be_var"
   {
	 ...
	 ERR;
      ...
      if ((native_did = H5Dopen(grpid, NATIVE_VAR_NAME)) < 0) ERR;
	 ...
	 if (data[i] != data_in[i]) ERR;
      ...
      if ((le_did = H5Dopen(grpid, LE_VAR_NAME)) < 0) ERR;
	 ...
	 if (data[i] != data_in[i]) ERR;
      ...
      if ((be_did = H5Dopen(grpid, BE_VAR_NAME)) < 0) ERR;
