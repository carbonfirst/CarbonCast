...
$Id: nc4internal.c,v 1.73 2008/06/06 19:52:22 ed Exp $
*/
...
/* Given a varid, find it's shape. For unlimited dimensions, return
		   ...
		   int *dimid, size_t *dimlen)
{
   ...
   if (dimlen)
   {
      /* If the var hasn't been created yet, it's size is 0. */
      if (!var->created)
      {
	 for (d = 0; d < var->ndims; d++)
	    dimlen[d] = 0;
	 ...
	 else
	 {
	    ...
	    if (dataset_ndims != *ndims)
	       BAIL(NC_EHDFERR);
	    if (!(h5dimlen = nc_malloc(dataset_ndims * sizeof(hsize_t))))
	       BAIL(NC_ENOMEM);
	    if (!(h5dimlenmax = nc_malloc(dataset_ndims * sizeof(hsize_t))))
	       BAIL(NC_ENOMEM);
  ...
  exit:
   ...
   if (datasetid > 0 && H5Dclose(datasetid) < 0)
      BAIL2(NC_EHDFERR);
   if (h5dimlen) nc_free(h5dimlen);
   if (h5dimlenmax) nc_free(h5dimlenmax);
   return retval;
}
...
nc4_nc4f_list_add(NC_FILE_INFO_T *nc, const char *path, int mode)
{
   ...
   /* The NC_FILE_INFO_T was allocated and inited by
      ncfunc.c before this function is called. We need to nc_malloc and
      initialize the substructure NC_HDF_FILE_INFO_T. */
   if (!(nc->nc4_info = nc_calloc(1, sizeof(NC_HDF5_FILE_INFO_T))))
      return NC_ENOMEM;
   h5 = nc->nc4_info;
    ...
    * it's hdf id, name, and a pointer to it's file structure. */
...
/* Given an ncid and varid, get pointers to the NC_FILE_INFO_T and
   NC_VAR_INFO_T metadata. */
int
		  ...
		  NC_GRP_INFO_T **grp, NC_VAR_INFO_T **var)
{
   ...
   *grp = nc4_rec_find_grp(nc->nc4_info->root_grp, (ncid & GRP_ID_MASK));
   ...
   /* Find the var info. The number of elements we'll eveutually
      read/write is the product of the count for each dimension. */
   for ((*var)=(*grp)->var; (*var); (*var)=(*var)->next)
     if ((*var)->varid == varid)
       break;
   if (!(*var))
...
nc4_rec_find_hdf_type(NC_GRP_INFO_T *start_grp, hid_t target_hdf_typeid)
{
   ...
   {
      if ((equal = H5Tequal(type->hdf_typeid, target_hdf_typeid)) < 0)
	 return NULL;
      if (equal)
	 return type;
   }
...
/* Use a typeid to find a type in a type_list. */
int
nc4_find_type(NC_HDF5_FILE_INFO_T *h5, int typeid, NC_TYPE_INFO_T **type)
{
   if (typeid < 0 || !type)
      return NC_EINVAL;
   *type = NULL;
...
nc4_find_dim_len(NC_GRP_INFO_T *grp, int dimid, size_t **len)
{
   ...
   for (var = grp->var; var; var = var->next)
   {
	 ...
	 return retval;
       ...
       * if it's length is longer than *lenp. */
      for (d = 0; d < ndims; d++)
      {
	 if (dimids[d] == dimid)
	 {
...
/* This will insert an NC_FILE_INFO_T at the beginning of the
   linked-list nc_file. */
int
nc4_file_list_add(int new_id)
{
   NC_FILE_INFO_T *nc;
   if (!(nc = nc_calloc(1, sizeof(NC_FILE_INFO_T))))
      return NC_ENOMEM;
   if(nc_file != NULL)
      nc_file->prev = nc;
   nc->next = nc_file;
...
nc4_var_list_add(NC_VAR_INFO_T **list, NC_VAR_INFO_T **var)
{
   ...
   if (!(*var = nc_calloc(1, sizeof(NC_VAR_INFO_T))))
      return NC_ENOMEM;
...
{
   NC_DIM_INFO_T *dim;
   if (!(dim = nc_calloc(1, sizeof(NC_DIM_INFO_T))))
      return NC_ENOMEM;
   if(*list)
      (*list)->prev = dim;
   dim->next = *list;
...
nc4_att_list_add(NC_ATT_INFO_T **list)
{
   ...
   if (!(att = nc_calloc(1, sizeof(NC_ATT_INFO_T))))
      return NC_ENOMEM;
   if (*list)
   {
		 ...
		 char *name, NC_GRP_INFO_T **grp)
{
   ...
   if (!(*grp = nc_calloc(1, sizeof(NC_GRP_INFO_T))))
      return NC_ENOMEM;
...
int
nc4_type_list_add(NC_TYPE_INFO_T **list, int typeid, size_t size, 
		  const char *name, nc_type class, nc_type base_nc_type)
{
   NC_TYPE_INFO_T *type, *t;

   if (!(type = nc_calloc(1, sizeof(NC_TYPE_INFO_T))))
      return NC_ENOMEM;
   ...
   /* Remembe info about this new type. */
   type->class = class;
   type->nc_typeid = typeid;
...
int
nc4_field_list_add(NC_FIELD_INFO_T **list, int fieldid, char *name,
		   ...
		   int ndims, int *dim_sizesp)
{
   ...
   if (!(field = nc_calloc(1, sizeof(NC_FIELD_INFO_T))))
      return NC_ENOMEM;
		    ...
		    const char *name, const void *value)
{
   ...
   /* Allocate storage for this field information. */
   if (!(member = nc_calloc(1, sizeof(NC_ENUM_MEMBER_INFO_T))) ||
       !(member->value = nc_calloc(1, size)))
      return NC_ENOMEM;
...
/* Delete a var from a var list, and nc_free the memory. */
static void
...
static void
dim_list_del(NC_DIM_INFO_T **list, NC_DIM_INFO_T *dim)
{
...
nc4_rec_grp_del(NC_GRP_INFO_T **list, NC_GRP_INFO_T *grp) 
{
   ...
   while (att)
   {
      ...
      a = att->next;
      if (att->hdf_typeid && H5Tclose(att->hdf_typeid) < 0)
	 return NC_EHDFERR;
   ...
   while (var)
   {
	 ...
	 return NC_EHDFERR;
      if (var->hdf_typeid && H5Tclose(var->hdf_typeid) < 0)
	 return NC_EHDFERR;
      v = var->next;
      var_list_del(&grp->var, var);
      var = v;
   }
...
nc4_att_list_del(NC_ATT_INFO_T **list, NC_ATT_INFO_T *att)
{
   ...
   /* Nc_Free memory that was nc_malloced to hold data for this
    * attribute. */
   if (att->data)
      nc_free(att->data);
