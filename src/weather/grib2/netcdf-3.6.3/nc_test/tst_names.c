/* This is part of the netCDF package.
   ...
   $Id: tst_names.c,v 1.7 2008/06/05 17:01:29 russ Exp $
*/
#include <config.h>
#include <stdlib.h>
#include <stdio.h>
...
main(int argc, char **argv)
{
   ...
   int i, j;
#define NUM_BAD (sizeof badnames / sizeof badnames[0])
#define NUM_GOOD (sizeof goodnames / sizeof goodnames[0])
   int ncid, dimid, varid, res;
   double attvals[] = {-2.0};
   double attvals_in[1];
#define NATTVALS (sizeof attvals / sizeof attvals[0])
   char *attstring = "text";
#define ATTSTRING_LEN 5
   char attstr_in[ATTSTRING_LEN];
   int dimids[NUM_GOOD];
   int varids[NUM_GOOD];
	...
	* acceptable names */
	   ...
	   if ((res = nc_def_dim(ncid, goodnames[i], DIMLEN, &dimid)))
	       ERR;
	   dimids[i] = dimid;
	   /* Define variable with same name */
	   if ((res = nc_def_var(ncid, goodnames[i], NC_FLOAT, NDIMS, &dimids[i], 
				 &varid)))
	       ERR;
	   varids[i] = varid;
	   /* Define variable and global attributes with same name */
	   if ((res = nc_put_att_text(ncid, varid, goodnames[i], 
				      strlen(attstring), attstring)))
	       ERR;
	   if ((res = nc_put_att_double(ncid, NC_GLOBAL, goodnames[i], NC_DOUBLE, 
					NATTVALS, attvals)))
	       ERR;
	   attnums[i] = i;
       }
	...
	* bad names and make sure these are rejected */
       for (i = 0; i < NUM_BAD; i++) {
	   if ((res = nc_def_dim(ncid, badnames[i], DIMLEN, &dimid)) 
	       != NC_EBADNAME) ERR;
	   if ((res = nc_def_var(ncid, badnames[i], NC_FLOAT, NDIMS, dimids, 
				 &varid))
	       != NC_EBADNAME) ERR;
	   if ((res = nc_put_att_text(ncid, varid, badnames[i], 
				      strlen(attstring), attstring))
	       != NC_EBADNAME) ERR;
	   if ((res = nc_put_att_double(ncid, NC_GLOBAL, badnames[i], NC_DOUBLE, 
					NATTVALS, attvals))
	       != NC_EBADNAME) ERR;
       }
       ...
       for (i = 0; i < NUM_GOOD; i++) {
	   size_t attlen;
	   if ((res = nc_inq_dimid(ncid, goodnames[i], &dimid)) || 
	       dimid != dimids[i])
	       ERR;
	   if ((res = nc_inq_varid(ncid, goodnames[i], &varid)) || 
	       varid != varids[i])
	       ERR;
	   res = nc_inq_attlen(ncid, varid, goodnames[i], &attlen);
	   if ((res = nc_get_att_text(ncid, varid, goodnames[i], attstr_in))) 
	       ERR;
	   attstr_in[attlen] = '\0';
	   if (strcmp(attstring, attstr_in) != 0) 
	       ERR;
	   if ((res = nc_get_att_double(ncid, NC_GLOBAL, goodnames[i], 
					attvals_in)) 
