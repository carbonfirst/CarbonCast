/*
  ...
  $Id: bm_file.c,v 1.57 2008/01/16 13:26:46 ed Exp $
*/
#include <config.h>
#include <stdio.h>
#include <string.h>
...
#define CHL
#define CHL_ONE_BIG_CALL
#define CHL_REUSE_HDF5FD
#define CHL_HDF5APITIME
#define CHL_PUREHDF5
#define CHL_CONTIGUOUS
#define CHL_NETCDF4_BIGENDIAN
#define CHL_REPEAT_COMPARISON

#ifdef CHL_ONE_BIG_CALL
static int chl_one_big_call = 0;
#endif
#ifdef CHL_PUREHDF5
static int chl_pure_hdf5 = 0;
static int chl_pure_hdf5_conversion = 0;
#endif

#ifdef CHL_REUSE_HDF5FD
static int chl_reusehdf5fd = 0;
void set_use_hdf5fds_cache(int f);
void close_all_hdf5fds();
struct timeval get_hdf5apitime();
#endif
#ifdef CHL_CONTIGUOUS
static int chl_contiguous = 0;
void set_force_contiguous(int f);
#endif
#ifdef CHL_NETCDF4_BIGENDIAN
static int chl_nc4_be = 0;
#endif
#ifdef CHL_REPEAT_COMPARISON
static int chl_repeat_comparison = 1;
#endif
...
main(int argc, char **argv)
{
	 ...
	 case 'c':
	    for (num_vo = 0, str1 = optarg; ; num_vo++, str1 = NULL) 
	    {
		  ...
		  if (str2)
		     ...
		     sscanf(subtoken, "%d", &(vo[num_vo].chunksize[ndims++]));
               }
   ...
   if (!my_rank)
   {
	 ...
	 printf("%d, %d, %d, %d, %d, %d ", vo[o1].deflate_num, vo[o1].shuffle, 
		vo[o1].chunksize[0], vo[o1].chunksize[1], vo[o1].chunksize[2], vo[o1].chunksize[3]);
	 if (o1 != num_vo - 1)
	    printf(", ");
      }
      printf("\n");
