...
# ENTSOE_BAL_AUTH_LIST = ['ES'] # ['DK', 'DE', 'FR', 'RO']
...
def calculateMissingSourceData(ba, prodDF, fcstDF): # FIX: RO and ES to change intervals after the dates
    ...
    if (prodDF is not None): # send if statements for time intervals for ES and RO (changing time intervals)
        ...
        for source in prodDF.columns:
            if (source != "UTC Time"):
                prodDF.loc[len(prodDF)-1, source] = round(((prodDF.loc[len(prodDF)-1, column])/TOTAL_MINS*100), 4) # out of all possible "source data"
    ...
    if (fcstDF is not None):
        ...
        for source in fcstDF.columns:
            if (source != "UTC Time"):
                fcstDF.loc[len(fcstDF)-1, source] = round(((fcstDF.loc[len(fcstDF)-1, column])/TOTAL_MINS*100), 4) # out of all possible "source data"
    print(prodDF, fcstDF)
    return prodDF, fcstDF
