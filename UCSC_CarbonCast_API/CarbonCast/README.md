...
## Quick Start: DB-Backed API And Pipeline

The API now reads carbon intensity, energy mix, weather forecasts, and 168-hour forecasts from PostgreSQL and falls back to legacy CSVs only when DB rows are missing. SQLite is only a legacy/import source for old local data. Follow these steps for a clean local run:

> Supported Python: 3.8-3.10 (tested on 3.10). TensorFlow 2.9.x and NumPy 1.23.x in `requirements.txt` do not support Python 3.11+; installs on newer runtimes fail with `pkgutil.ImpImporter`/wheel build errors. Use a 3.10 virtualenv (Apple Silicon uses `tensorflow-macos`, other platforms use `tensorflow`).

1. Create Python 3.10 virtual environment & install deps:
  ```bash
  brew install python@3.10  # or: pyenv install 3.10.14
  python3.10 -m venv .venv
  source .venv/bin/activate
  pip install --upgrade pip setuptools wheel
  pip install -r requirements.txt
  ```
2. Run migrations (creates tables for models like EmissionActual, WeatherForecast, ModelRun, and Forecast96 in [`models.py`](src/CarbonCastAPI/CarbonCastRESTAPI/models.py:32)):
  ```bash
...
3. (Optional first run) Create a test user for authenticated endpoints:
  ...
  ```
4. (Optional and needed if new data needs to be added/ingested into API) Import real-time actual and forecast CSVs (idempotent upsert via [`import_csvs`](src/CarbonCastAPI/CarbonCastRESTAPI/management/commands/import_csvs.py:1)):
  ```bash
...
Register recurring pipeline schedules with:
```bash
python src/CarbonCastAPI/manage.py setup_pipeline_schedules
```

For detailed operational notes see [`docs/API_instructions.md`](docs/API_instructions.md:1) and [`docs/realtime_pipeline_v1.md`](docs/realtime_pipeline_v1.md:1).
...
CarbonCast requires Python3 (3.8-3.10; tested on 3.10). Python 3.11+ is not supported by the pinned TensorFlow/NumPy versions. <br>
Other required packages:
* Required python modules are listed in ```requirements.txt```.<br>
Run ```python3.10 -m pip install -U -r requirements.txt``` (or the matching Python 3.8/3.9 interpreter) for installing the dependecies.
